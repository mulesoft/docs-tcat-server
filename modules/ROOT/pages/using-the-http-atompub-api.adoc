...
To use the AtomPub API with your Java application, we recommend using the http://incubator.apache.org/abdera/[Apache Abdera framework]. Other tools like `wget` will work as well. For a more visual guidance, check out the lnk:http://www.xucia.com/#RestTest[RestTest]FireFox plug-in.
