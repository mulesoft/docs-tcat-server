...
For a complete and working example of a Maven POM file that you can download and try, download link:{attachmentsdir}/tcat-console-webapp-upload-maven.zip[the Tcat Maven antrun plugin example zip file] and unpack it. The POM file XML that performs the continuous deployment looks like this:
...
After using the plug-in to update your applications, you can schedule a script to listen for these updates and automatically redeploy their packages. For an example, see xref:scripting-examples.adoc[Automatically Redeploying After an Artifact Update].
...
xref:repository-api.adoc[<< Previous: *Using REST API to Manage Tcat Repository*]

xref:release-notes.adoc[Next: *Release Notes* >>]
