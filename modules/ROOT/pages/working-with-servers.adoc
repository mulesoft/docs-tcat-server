...
This page describes how to work with servers. For information on monitoring a server's details, see xref:monitoring-a-server.adoc[Monitoring a Server].
...
For information on creating a server profile, see xref:working-with-server-profiles.adoc[Working with Server Profiles].

== Restarting a Server

You can restart one or more servers by selecting their check boxes and clicking *Restart*. You can also restart a server when you are xref:monitoring-a-server.adoc[viewing its details].
...
You can now start adding servers to this group. You can also xref:deploying-applications.adoc[deploy packages] to the group, allowing you to deploy all the package's applications to all the group's servers in one step.
...
For additional troubleshooting tips, see xref:troubleshooting.adoc[Troubleshooting].

xref:deploying-to-amazon-ec2.adoc[<< Previous: *Deploying to Amazon EC2*]
