...
You manage users and user groups on the Administration tab in Tcat Server. You can also use LDAP to enable authentication (see xref:integrating-with-ldap.adoc[Enabling Authentication Through LDAP]). Note that after enabling authentication through LDAP, the Users link is no longer available.
...
xref:user-group-permissions.adoc[<< Previous: *User Group Permissions*]
