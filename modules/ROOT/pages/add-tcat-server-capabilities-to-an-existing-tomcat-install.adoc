...
|Any operating system that has a fully compliant Java 1.5 or newer JDK or JRE.  See xref:installation.adoc[Java Runtime Requirements] for more details.
|===
...
If so, consult this section of the xref:installing-multiple-tcat-instances-on-a-single-machine.adoc[Tcat Server Installation documentation].
